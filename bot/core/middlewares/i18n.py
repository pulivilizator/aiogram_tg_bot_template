...
class TranslatorRunnerMiddleware(BaseMiddleware):
    ...
    @staticmethod
    async def _get_lang(cb: CallbackQuery, user: User, cache: UserCache, update_settings: UpdateUserSettingsInteractor) -> str:
        if cb and Languages.WIDGET_KEY in cb.data:
            ...
            await update_settings.execute(settings_id=str(cache.settings.id), data=dto.UpdateUserSettingsDTO(language=lang))
            return lang
