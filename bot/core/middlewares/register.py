...
class RegisterMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        from_user: User | None = getattr(event, 'from_user', None) or getattr(event, 'event_from_user', None)
                ...
                user = await create_user.execute(
                    data=dto.CreateUserDTO(
                        ...
                        settings=dto.UserSettingsDTO(
                            language=Languages.RU if from_user.language_code == Languages.RU else Languages.EN,
    ...
    @staticmethod
    async def update_cache(cache: UserCache,
                           db_user: dto.UserWithSettingsDTO,
                           tg_user: User):
        await cache.settings.id.set(str(db_user.settings.id))
        await cache.settings.language.set(Languages.RU if tg_user.language_code == Languages.RU else Languages.EN)